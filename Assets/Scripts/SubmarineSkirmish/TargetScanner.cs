...
public class TargetScanner
{
    ...
    public BHPlayerController Detect(Transform detector, bool useHeightDifference = true)
    {
        //if either the player is not spwned or they are spawning, we do not target them
        if (BHPlayerController.instance == null)
            return null;
            
        Vector3 eyePos = detector.position + Vector3.up * heightOffset;
        Vector3 toPlayer = BHPlayerController.instance.transform.position - eyePos;
        Vector3 toPlayerTop = BHPlayerController.instance.transform.position + Vector3.up * 1.5f - eyePos;
        ...
        if (toPlayerFlat.sqrMagnitude <= detectionRadius * detectionRadius)
        {
            if (Vector3.Dot(toPlayerFlat.normalized, detector.forward) >
                Mathf.Cos(360 * 0.5f * Mathf.Deg2Rad))
            {
                ...
                if (canSee)
                    return BHPlayerController.instance;
            }
        }
...
public void EditorGizmo(Transform transform)
{
    ...
    Vector3 rotatedForward = Quaternion.Euler(0, -360 * 0.5f, 0) * transform.forward;
    UnityEditor.Handles.DrawSolidArc(transform.position, Vector3.up, rotatedForward, 360, detectionRadius);

    c = new Color(0.7f, 0, 0, 0.4f);
    UnityEditor.Handles.color = c;
    UnityEditor.Handles.DrawSolidArc(transform.position, Vector3.up, rotatedForward, 360, attackRadius);
